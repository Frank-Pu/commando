...
**已有**：install + onboard + build-skills + go-live + schedule + 飞书 IM + Obsidian + 本地 dashboard + Registry（3 个已验证的通用 Skill）。经过 3 轮陌生人上手 dogfood 验证。

**未做**：Linux `systemd` 模板（目前只 macOS 的 launchd 跑通）、更多第三方 agent CLI 的实测命令格式、Stripe / Plausible KPI 看板。

### ⚠ 运行时门槛（重要，请别跳过）

交互式部分（`commando onboard`、`commando build-skills`）任意 AI 工具都能跑——CLI 或 IDE 都行。但**定时任务由 launchd / systemd / cron 触发**，需要一个能 **headless** 跑的 LLM 后端：

- **PATH 里装一个 CLI agent** —— `claude`（Claude Code）、`codex`、`kimi`、`glm`、`qwen`、`doubao`、`minimax`、`gemini` 任选其一，且该 CLI 已登录（auth 能传给 subprocess）。
- **或者一个 API key** —— shell rc 里 `export ANTHROPIC_API_KEY=...` 或 `ZHIPU_API_KEY=...`。Anthropic 充值 5 美元够用几个月轻量使用。

**只用 IDE agent（Cursor / Windsurf / Trae / VS Code Copilot / JetBrains AI）跑不了定时任务**——IDE 关掉 agent 就不存在了。IDE 适合 Onboarding 阶段；想让搭档在你不在电脑前时自己上岗，必须再补一个上面的 headless 路径。

两个都没配的话，`commando build-skills --apply` 和定时任务触发都会失败，并提示 "no LLM backend available"。

...
在讨论"Skill 启用清单"这个决议时，**必须用 Read 工具读
`commando/skills.json` 的当前社区 Skill 索引**，主动告诉用户：

> 你描述的 <X 公式 / Y 流程>，社区已经有 `@<author>/<skill-name>`
> 这个 Skill 实现了类似功能，要不要直接 import？这样你可以省 N 小时
> 自己写 + 拿到一份已经被验证过的实现。

这是 commando 社区飞轮的核心。**不主动提，等于背叛了 commando 的产品设计**。
...
2. 对每个 Skill 占位：
   ...
   - 待开发的，frontmatter 里写 `status: draft`，body 里写 Charter
     描述的预期能力 + 主要输入输出契约草稿
